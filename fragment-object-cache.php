<?php 
  ...
  /*
    Plugin Name: Fragment and object Cache
    Description: Fragment and Object Caching using WP Transients API.
    Author: Sushant
    License: GPL v2
    
    Fragment and object Cache Plugin
  */
	
	// If this file is called directly, abort.
	if (!defined('WPINC')) {die;}
  
  if (!class_exists('frag_obj_cache')) {
  
    class frag_obj_cache {
      
      private $frag_keys = array();
      private $frag_keys_option = '_fragobj_cache_frag_keys_';  // option to store frag keys
      
      private $object_keys = array();
      private $object_keys_option = '_fragobj_cache_object_keys_'; // option to store object keys
      ...
      public function __construct() {
        ...
        add_filter('fragobj_cache_get_object', array($this, 'object_get'), 10, 2);
        add_action('fragobj_object_save', array($this, 'object_save'), 10, 3);
        
        add_filter('fragobj_cache_frag_check', array($this, 'frag_check'), 10, 2);
        add_action('fragobj_cache_frag_output_save', array($this, 'frag_output_save'), 10, 2);
        
        add_action('fragobj_cache_uncache', array($this, 'clear_item'), 10, 2);
				
				add_action('after_setup_theme', array($this, 'add_clear_actions'));
				add_action('fragobj_cache_clear', array($this, 'clear_cache'));
      ...
      public function activate() {
        // just in case I want to do anything on activate
			...
			public function add_clear_actions() {
				...
				if (!defined('FRAGOBJ_CACHE_CLEAR_ACTIONS')) {
					add_action('admin_init', array($this, 'clear_cache'));
					return;
				}
				if (defined('FRAGOBJ_CACHE_CLEAR_ACTIONS') && FRAGOBJ_CACHE_CLEAR_ACTIONS === false) {
					return;
				}
				$actions = explode(',', FRAGOBJ_CACHE_CLEAR_ACTIONS);
      ...
      public function init() {
        ...
        if (isset($_GET['fraobj-cache']) && strtolower($_GET['fraobj-cache']) == 'clear') {
          $this->clear_cache();
          $redirect = remove_query_arg('fraobj-cache');
      ...
      public function set_default_ttl() {
        $ttl = intval(apply_filters('fraobj_cache_ttl', $this->ttl));
        if ($ttl) {
          $this->ttl = $ttl;
        }
      ...
      private function get_frag_keys() {
        // get list of cache keys that fragobj cache is tracking from options
      ...
      private function save_frag_keys() {
        // save list of cache keys that fragobj cache is tracking in options
    ...
    } // end class fragobjSnippets
    
    new frag_obj_cache();
    
  } // end if (!class_exists('frag_obj_cache'))
